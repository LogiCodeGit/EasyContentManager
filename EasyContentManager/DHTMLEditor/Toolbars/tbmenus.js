// Copyright 2000-2002 Microsoft Corporation. All rights reserved. 
...
function TBPopUpMenu(menu, nestingDepth) {
  ...
  try {
      ...
      if (bUseItalian)
          alert("Questa funzionalità richiede l'abilitazione delle scriptlet nelle opzioni del browser...");
      else
          alert("This functionality require to enable scriptlet in your browser options...");
  }
  ...
  switch (eventName) {
      case "TBMenuClick":
          ...
          if (element.TBUSERONCLICK) {
              //alert(element.TBUSERONCLICK);
              //eval(element.TBUSERONCLICK + " anonymous();");
              eval(element.TBUSERONCLICK + "  try{anonymous();}catch(e){onclick();}");
          }
          break;
...
//  document.write('<SCRIPT LANGUAGE="JavaScript" FOR=TBSCRIPTLET EVENT="onscriptletevent(eventName, eventObject)"> TBScriptletEvent(this, eventName, eventObject); </scrip' +'t>');
    document.write('<script language="JavaScript" for=TBSCRIPTLET event="onreadystatechange"> TBScriptletReadyState(this); </script>');
    document.write('<script language="JavaScript" for=TBSCRIPTLET event="onscriptletevent(eventName, eventObject)"> TBScriptletEvent(this, eventName, eventObject); </script>');
}
// Add onload event
//try {
